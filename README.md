...
검사지 데이터를 다루며 실제로 겪은 것들입니다. 같은 데서 다시 막히지 않도록 회귀 테스트로 고정해 뒀습니다.

**이름이 겹치는 다른 검사.** 요침사 `WBC(/HPF)`는 혈액 `WBC`가 아니고, `풍진항체(IgM)`는 `IgG`와 완전히 다른 검사입니다. 전자는 최근 감염, 후자는 과거 면역 여부를 봅니다. 괄호를 떼어내 매칭하면 단위도 기준도 다른 항목에 붙어 판정이 통째로 뒤집힙니다.

**보이지 않는 문자 차이.** 마이크로는 코드포인트가 셋입니다 — `µ`(U+00B5), `μ`(U+03BC), `㎕`(U+3395). 눈으로는 같은데 문자열로는 다릅니다. `10^3/µL`과 `10^3/uL`이 다른 단위로 갈려 판정이 막힌 적이 있습니다.

**한계치 표기.** `양성(>500)`의 `>500`은 측정치가 아니라 "500을 넘었다"는 말입니다. `500`으로 저장하면 추이 그래프에 없는 수치가 찍힙니다. 판정만 남기고 원문은 `raw_value`에 보존합니다.

**단위 자릿수.** `mIU/L`과 `mIU/mL`은 1000배 차이입니다. 표기가 비슷해 같은 단위로 접어뒀다가 TSH 수치에 틀린 단위가 붙는 걸 발견했습니다.

**React Native의 FormData.** 파일 업로드를 `@RequestPart`로 받으면 안 됩니다. RN은 `{ uri, name, type }` 객체를 파일로 보내면서 파트 헤더를 브라우저와 다르게 실어, Spring이 파트를 아예 못 찾습니다. `@RequestParam`을 씁니다.
