...
병원에서 받은 산전검사지에는 아래와 같은 문장이 적혀있습니다.
...
임신하면 혈장량이 크게 늘어 혈색소가 자연히 희석됩니다. 학회 기준은 주수에 따라 `10.5~14.0` 수준인데, 검사지에 인쇄된 `12.0~16.0`은 임신하지 않은 성인 기준이기 때문입니다. 검사지만 보면 정상인 사람이 빈혈 환자가 됩니다.

반대인 경우도 있습니다. 검사지 기준으로는 정상인데 임신 중 기준으로는 조치가 필요한 항목도 있습니다. 어느 쪽이든 인쇄된 참고치를 그대로 믿어서는 안됩니다.
...
핵심은 `ResultEvaluator`의 순서입니다. 카탈로그에 `is_pregnancy_specific`이 켜진 항목은 검사지에 인쇄된 참고치를 무시하고 임신 기준을 사용한다. 앞의 혈색소 사례가 여기서 갈린다.

<br/>

## 설계에서 지킨 것

**판정은 AI가 하지 않습니다.** 수치 판정은 전부 코드가 결정론적으로 계산한다. AI는 사진에서 글자를 옮겨 적고, 이미 나온 판정을 문장으로 풀어쓰는 데만 사용한다. 같은 검사지를 두 번 올리면 같은 답이 나와야 하고 왜 그 판정인지 근거를 댈 수 있어야 하는데, 생성형 모델은 둘 다 보장하지 못한다.

**애매하면 이상 쪽으로 기웁니다.** 판정 기준을 모르거나 값을 못 믿겠으면 정상이라고 추측하지 않고 "확인 필요"로 둔다. 이상 수치를 정상이라고 말하면 병원에 가야 할 사람이 안 가게 되지만, 정상을 확인 필요라고 하면 한 번 더 물어보게 될 뿐이다. 두 실패의 무게가 다르니 기울일 방향도 정해져 있다.

**못 읽은 항목도 버리지 않습니다.** 카탈로그에 매칭되지 않은 줄도 원문을 그대로 저장한다. 나중에 항목 표기를 보강하면 살릴 수 있다. 지워버리면 사용자가 검사지를 다시 꺼내야 한다.
...
`compat`이 따로 있는 이유는 앱이 자체 판정 엔진을 갖게 되면서 역할이 갈렸기 때문이다. 엔진이 판정한 항목은 앱 판정을 그대로 쓰고, 엔진이 모르는 항목만 서버 판정을 폴백으로 쓴다. 어느 쪽을 쓰든 화면에 보이는 값과 DB에 남는 값은 항상 같다. 다르면 업로드 직후엔 "안심"이던 항목이 기록 탭에서 다른 상태로 바뀐다.
...
전체 명세는 서버를 띄운 뒤, `/swagger-ui.html`에서 볼 수 있다.

### 검사지를 올리는 두 가지 경로

`POST /test-sheets`는 사진을 서버에 올리면 서버가 OCR부터 판정까지 한다. 즉시 202를 주고 `GET /{id}/status`를 2초 간격으로 폴링한다.

`POST /reports`는 앱이 OCR과 판정을 끝낸 뒤 결과만 보낸다. 현재 앱이 쓰는 경로이다. 사진이 없으므로 저장 후 `POST /test-sheets/{id}/images`로 원본을 따로 붙인다.

`/reports` 응답은 요청과 같은 개수·같은 순서를 보장한다. 앱이 배열 인덱스로 짝짓기 때문에, 서버가 정렬이나 중복 제거를 한 번만 해도 다른 항목의 이름이 붙는다. 인덱스 대신 쓸 수 있도록 응답 항목마다 `resultId`를 함께 내려준다.
...
스키마와 시드는 `src/main/resources/db/`에 있습니다. 순서대로 실행한다.
...
`ddl-auto: validate`라 스키마가 엔티티와 어긋나면 기동 자체가 막힌다. 마이그레이션을 건너뛰면 서버가 뜨지 않는다.
...
파서·매처 테스트는 단독으로 돌지만, `TestSheetAnalyzerTest`는 시드 카탈로그를 쓰므로 MySQL·Redis가 떠 있어야 한다.
...
`name_variants`가 병원마다 다른 표기를 흡수하고, `hard_limit_min/max`는 OCR이 자릿수를 잘못 읽었을 때(`10.8` → `108`) 걸러내게 된다.

`test_results`는 검사 당시의 `test_date`와 `pregnancy_week`를 복사해 둔다. 정규화 관점에서는 중복이지만 판정 시점의 기준이 보존된다. 
<br/>
